...
__fnm_initialized=0
__init_fnm_once() {
  [ "$__fnm_initialized" -eq 1 ] && return 0
  __fnm_initialized=1
...
if has fnm; then
  # Alias-based wrappers are more compatible than node() function declarations.
  __fnm_exec() {
    __init_fnm_once
    command "$@"
  }
  ...
  alias npm='__fnm_exec npm'
  alias npx='__fnm_exec npx'
  alias pnpm='__fnm_exec pnpm'
  alias yarn='__fnm_exec yarn'
fi
